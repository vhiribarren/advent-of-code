...
const INPUT_FILENAME: &str = "day_11.txt";
...
fn paint_grid(program: &str, start_input: usize) -> (usize, HashSet<Coords>) {
    let mut visited = HashSet::<Coords>::new();
    let mut whites = HashSet::<Coords>::new();
    let mut computer = IntCodeComputer::from_input(program);
    let mut robot = (0, 0);
    ...
    while !computer.is_halted() {
        let in_color = if whites.contains(&robot) { 1 } else { 0 };
        let &[out_color, out_direction, ..] = computer.run_program(vec![in_color]).as_slice()
        else {
            panic!("Vector to array coercition failed");
        };
        ...
        if out_direction.0 == 0 {
            direction = direction.left();
        } else {
            direction = direction.right();
        }
        robot = (
            robot.0 + direction.vector().0,
            robot.1 + direction.vector().1,
        );
    }
    (visited.len(), whites)
}
...
fn problem_1(input: &str) -> Result<String, Box<dyn Error>> {
    let (count, _) = paint_grid(input, 0);
    Ok(count.to_string())
}

fn problem_2(input: &str) {
    let (_, whites) = paint_grid(input, 1);
    let width = whites.iter().map(|(w, _)| *w).max().unwrap();
    let height = whites.iter().map(|(_, h)| *h).max().unwrap();
    for y in 0..=height {
        for x in 0..=width {
